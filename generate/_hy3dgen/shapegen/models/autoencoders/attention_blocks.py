...
# [OPTIMIZACIÓN]: einops eliminado. Se usa .transpose nativo para no asfixiar la CPU.
...
class QKVMultiheadCrossAttention(nn.Module):
    ...
    def forward(self, q, kv):
        ...
        # [OPTIMIZACIÓN]: .transpose nativo en lugar de einops.rearrange
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
...
def fps(
    ...
    # [OPTIMIZACIÓN]: .contiguous() vital para que el C++ de torch_cluster no fragmente memoria
    src = src.float().contiguous()
...
class PointCrossAttentionEncoder(nn.Module):
    ...
    def sample_points_and_latents(self, pc: torch.FloatTensor, feats: Optional[torch.FloatTensor] = None):
        ...
        # [OPTIMIZACIÓN]: device=pc.device en lugar de .to(pc.device) para ahorrar ciclos
        batch_down = torch.arange(B, device=pc.device)
        ...
        if input_sharpedge_pc_size == 0:
            # [OPTIMIZACIÓN]: device=pc.device 
            input_sharpedge_pc = torch.zeros((B, 0, D), dtype=input_random_pc.dtype, device=pc.device)
            query_sharpedge_pc = torch.zeros((B, 0, D), dtype=query_random_pc.dtype, device=pc.device)
        else:
        ...
        if self.point_feats != 0:
            ...
            if input_sharpedge_pc_size == 0:
                ...
                input_sharpedge_surface_feats = torch.zeros((B, 0, self.point_feats),
                                                            dtype=input_random_surface_feats.dtype, device=pc.device)
                query_sharpedge_feats = torch.zeros((B, 0, self.point_feats), dtype=query_random_feats.dtype, device=pc.device)
            else:
        ...
        if input_sharpedge_pc_size == 0:
            # [OPTIMIZACIÓN]: device=pc.device
            query_sharpedge_pc = torch.zeros((B, 1, D), device=pc.device)
            input_sharpedge_pc = torch.zeros((B, 1, D), device=pc.device)
    ...
    def forward(self, pc, feats):
        ...
        # [OPTIMIZACIÓN]: Eliminado `query = query`
        data = self.input_proj(data)
        # [OPTIMIZACIÓN]: Eliminado `data = data`
