...
class SurfaceExtractor:
    ...
    def __call__(self, grid_logits, **kwargs):
        ...
        for i in range(grid_logits.shape[0]):
            try:
                # Extraemos solo el logit actual para no saturar memoria
                ...
                # Limpiamos las variables pesadas del ciclo actual
                del vertices
                del faces
                del current_logit
                free_memory()
...
class MCSurfaceExtractor(SurfaceExtractor):
    def run(self, grid_logit, *, mc_level, bounds, octree_resolution, **kwargs):
        # 1. Pasamos el tensor a CPU y lo convertimos a numpy
        grid_logit_np = grid_logit.detach().cpu().numpy()
        
        # 2. Eliminamos inmediatamente el tensor original de la GPU para liberar VRAM
        del grid_logit
        free_memory()

        # 3. Ejecutamos Marching Cubes (esto consume mucha RAM del sistema)
        ...
        # 4. Destruimos el array de numpy gigante que ya no necesitamos
        del grid_logit_np
        del normals
        gc.collect()
...
class DMCSurfaceExtractor(SurfaceExtractor):
    def run(self, grid_logit, *, octree_resolution, **kwargs):
        ...
        # Ya no necesitamos grid_logit en VRAM
        del grid_logit 
        ...
        # Limpieza masiva de tensores intermedios en la GPU
        del sdf
        del verts
        free_memory()
