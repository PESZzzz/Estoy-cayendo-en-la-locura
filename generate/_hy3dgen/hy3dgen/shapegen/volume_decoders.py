...
class VanillaVolumeDecoder:
    ...
    def __call__(
        ...
        num_chunks: int = 65536,  # Batch masivo
...
class HierarchicalVolumeDecoding:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Chunk_size masivo
        chunk_size = max(num_chunks, 65536)

        # 2. latents to 3d volume (Paso 1: Grid inicial)
        batch_logits = []
        batch_size = latents.shape[0]
        for start in tqdm(range(0, xyz_samples.shape[0], chunk_size),
        ...
        for octree_depth_now in resolutions[1:]:
            ...
            batch_logits = []
            for start in tqdm(range(0, next_points.shape[0], chunk_size),
                              desc=f"Hierarchical Volume Decoding [r{octree_depth_now + 1}]", disable=not enable_pbar):
                queries = next_points[start: start + chunk_size, :]
                batch_queries = repeat(queries, "p c -> b p c", b=batch_size)
...
# Alias para resolver el ImportError en model.py manteniendo la optimización acelerada
