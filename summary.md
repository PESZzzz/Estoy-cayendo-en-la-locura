...
2. **Hybrid Strategy**  
   Runs heavy diffusion steps on your GPU for speed, but automatically offloads VRAM-hungry 3D Mesh Extraction to system RAM / CPU. This is the *"secret sauce"* that allows 4–8 GB VRAM laptops to complete generations without crashing.
...
4. **Anti-Memory-Bomb Patch**  
   Removes heavy `deepcopy` functions during image conditioning.
...
9. **Pre-allocated Memory Tensors**  
   Replaces memory-heavy `torch.cat()` operations during 3D volume decoding with pre-allocated tensors. Eliminates 200% memory spikes that previously caused OOM crashes.
